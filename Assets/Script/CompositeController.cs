...
public class CompositeController : MonoBehaviour
{
    ...
    private string RunPythonProcess(int r, int g, int b, string outputPath)
    {
        ...
        if (!useExeInEditor)
        {
            // Pastikan file python ada di folder backend
            // Note: User menyimpan file di "Assets/Python file/composite2_standalone.py"
            // Kita bisa arahkan ke sana, atau copy ke StreamingAssets.
            // Untuk amannya, kita asumsikan user sudah copy atau kita arahkan ke source.
            
            // Coba cari di StreamingAssets dulu
            string scriptPath = Path.Combine(backendFolder, pythonScriptName);
            if (!File.Exists(scriptPath))
            {
                // Fallback ke folder source dev jika ada
                string devPath = Path.Combine(Application.dataPath, "Python file", pythonScriptName);
